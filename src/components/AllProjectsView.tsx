        ...
        {view === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <button
              ...
              className="group flex flex-col items-center justify-center h-48 rounded-xl border-2 border-dashed border-border hover:border-primary/40 transition-colors bg-card/30"
            >
            ...
            {filtered.map((p) => (
              <button
                ...
                className="group text-left rounded-xl border border-border bg-card hover:border-primary/30 transition-all overflow-hidden"
              >
                {/* Preview thumbnail */}
                <div className="h-36 bg-muted/30 overflow-hidden relative">
                  ...
                  {p.publishedUrl && (
                    <span className="absolute bottom-2 left-2 text-[10px] px-2 py-0.5 rounded bg-primary/20 text-primary font-medium">
                      Published
                    </span>
                  )}
                  <span
                    onClick={(e) => handleDelete(e, p.id, p.name)}
                    className="absolute top-2 right-2 p-1.5 rounded-md bg-card/80 opacity-0 group-hover:opacity-100 text-destructive hover:bg-destructive hover:text-destructive-foreground transition-all"
                  >
          ...
          <div className="space-y-1">
            {filtered.map((p) => (
              <button
                key={p.id}
                onClick={() => onOpenProject(p)}
                className="w-full flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-card transition-colors group text-left"
              >
                ...
                <span
                  onClick={(e) => handleDelete(e, p.id, p.name)}
                  className="opacity-0 group-hover:opacity-100 p-1.5 rounded text-destructive hover:bg-destructive hover:text-destructive-foreground transition-all"
                >
                  <Trash2 className="h-3 w-3" />
                </span>
              </button>
