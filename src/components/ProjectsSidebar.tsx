...
const ProjectsSidebar = ({ isOpen, onClose, onNewProject, onOpenProject, activeProjectId }: Props) => {
  ...
  // Refresh projects when sidebar opens
  ...
  // Refresh on open
  if (isOpen && projects.length !== getProjects().length) {
    refreshProjects();
  }
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 z-50 h-full w-72 bg-card border-r border-border shadow-2xl transition-transform duration-300 flex flex-col ${
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4 space-y-1">
          {projects.length === 0 ? (
            <div className="text-center py-10 text-muted-foreground">
              <p className="text-xs">Nog geen projecten</p>
            </div>
          ) : (
            projects.map((p) => (
              <button
